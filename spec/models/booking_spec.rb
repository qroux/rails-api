...
RSpec.describe Booking, type: :model do
  ...
  it "CREATE: bookings for a different listing but same date should work" do
    Listing.create(num_rooms: 10)
    Booking.create(
      listing_id: Listing.last.id,
      start_date: Date.today,
  ...
  it "availability: Booking should work if already booked for another period" do
    ...
    Booking.create(
      listing_id: Listing.last.id,
      start_date: Date.new(2020,7,25),
      end_date: Date.new(2020,7,30)
    )
  ...
  it "UPDATE + availability: Booking update should fail if already booked(intersection)" do
    ...
    Booking.create(
      listing_id: Listing.last.id,
      start_date: Date.new(2019,7,25),
      end_date: Date.new(2019,7,30)
    )
